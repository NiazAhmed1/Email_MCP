...
@mcp.tool('fetch_unread_emails_last_2_days')
def fetch_unread_emails_last_2_days(folder: str = "INBOX", limit: int = 10) -> dict:
    """
    Fetch unread emails from the last 2 days in a specified folder (default: INBOX).
    
    Returns JSON with status and emails list.
    """
    try:
        mail = imaplib.IMAP4_SSL("imap.gmail.com")
        mail.login(SMTP_USERNAME, SMTP_PASSWORD)
